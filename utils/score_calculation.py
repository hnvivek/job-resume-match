...
def calculate_match_score(job_keywords, resume_keywords, resume_content):
    # Preprocess and extract hard and soft_skills from the dictionaries
    job_hard_skills = set(preprocess_keywords(job_keywords.get("hard_skills", [])))
    job_soft_skills = set(preprocess_keywords(job_keywords.get("soft_skills", [])))
    resume_hard_skills = set(preprocess_keywords(resume_keywords.get("hard_skills", [])))
    resume_soft_skills = set(preprocess_keywords(resume_keywords.get("soft_skills", [])))

    # Update the original dictionaries with preprocessed keywords
    job_keywords["hard_skills"] = list(job_hard_skills)
    job_keywords["soft_skills"] = list(job_soft_skills)
    resume_keywords["hard_skills"] = list(resume_hard_skills)
    resume_keywords["soft_skills"] = list(resume_soft_skills)

    # Combine hard and soft_skills into a single set for each
    ...
    # Calculate missing hard_skills and soft_skills
